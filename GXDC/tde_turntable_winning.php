...
$read_table = "tde_turntable_winning";//读取的表名
$table_field = "winningDate";//读取的字段名称
...
$txtFileName = "/toodo/crontab/txt/tdeTurntableWinning.txt";
$TxtRes = fopen($txtFileName, "w");

while ($row = mysqli_fetch_array($result)) {//----------------根据表字段数填写
    fwrite($TxtRes, $row[0] . "#" . $row[1] . "#" . $row[2] . "#" . $row[3] ."#" . $row[4] ."#" . $row[5] ."#" . $row[6] . "\r" . "\n");
}
fclose($TxtRes);
mysqli_close($conn);
...
date_default_timezone_set("Asia/Chongqing");
$create_table_name = "tde_turntable_winning";//创建的表名
...
if(mysqli_num_rows(mysqli_query($conn,"SHOW TABLES LIKE '". $create_table_name."'"))==1) {
    ...
    while (!feof($data)) {
        ...
        $lists = "insert into $create_table_name($field1,$field2,$field3,$field4,$field5,$field6,$field7) values ('$lines')";
    ...
    $createtable = "create table $create_table_name($field1 int(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,$field2 int(10),$field3 varchar(255),$field4 int(10),$field5 varchar(255),$field6 datetime,$field7 int(10)) default charset=utf8";
...
$insertShowtime = date("Y-m-d H:i:s");
$createShowtime = date("Ym");
$createTableDate = 'phpLoopRecords_tde_turntable_winning';
