...
$table_time1 = "tde_turntable_prize";//读取的表名
$table_field = "date";//读取的字段名称
...
//$sql = "select * from $table_time1 WHERE $table_field between '$startTime' AND '$stopTime'";
$sql = "select * from $table_time1";

$result = mysqli_query($conn,$sql);

$txtFileName = "/toodo/crontab/txt/tdeTurntablePrize.txt";
$TxtRes = fopen($txtFileName, "w");

while ($row = mysqli_fetch_array($result)) {//----------------根据表字段数填写
    fwrite($TxtRes, $row[0] . "#" . $row[1] . "#" . $row[2] . "#" . $row[3] . "\r" . "\n");
}
fclose($TxtRes);
mysqli_close($conn);
...
date_default_timezone_set("Asia/Chongqing");
$create_table_name = "tde_turntable_prize";//创建的表名

if(mysqli_num_rows(mysqli_query($conn,"SHOW TABLES LIKE '". $create_table_name."'"))==1) {

    $delTable = "delete from $create_table_name";
    mysqli_query($conn,$delTable);

    $createtable = "create table $create_table_name($field1 int(10) UNSIGNED PRIMARY KEY,$field2 varchar(255),$field3 datetime,$field4 varchar(255)) default charset=utf8";
    mysqli_query($conn,$createtable);

    while (!feof($data)) {
        $line = fgets($data);
        $lines = join("','", explode('#', $line));
        $lists = "insert into $create_table_name($field1,$field2,$field3,$field4) values ('$lines')";
...
$insertShowtime = date("Y-m-d H:i:s");
$createShowtime = date("Ym");
$createTableDate = 'phpLoopRecords_tde_turntable_prize';
